    ...
    <div>
      ...
      <div className="flex flex-col items-center gap-2">
        <div
          ...
          className="border-border/60 bg-muted/30 flex items-center rounded-full border p-1"
        >
      ...
      <div className="border-border/60 mt-10 grid grid-cols-1 rounded-2xl border lg:grid-cols-3 lg:[&>*:not(:first-child)]:border-l lg:[&>*]:border-border/60">
        {tiers.map((t) => (
          <div
            key={t.id}
            className={cn(
          ...
          >
            {t.highlight && (
              <span className="bg-foreground text-background absolute -top-2.5 left-7 rounded-full px-2 py-0.5 font-mono text-[10px] font-semibold tracking-wider uppercase lg:top-3.5">
                Most popular
              </span>
            )}
            <h3 className={cn("text-foreground text-lg font-semibold tracking-tight", t.highlight && "lg:mt-6")}>
              {t.name}
            </h3>
            ...
            <p className="text-foreground/80 mt-6 text-xs font-medium">{t.leadIn}</p>
      ...
      <div className="border-border/60 bg-muted/20 mt-12 flex flex-col items-center gap-4 rounded-2xl border border-dashed p-6 text-center sm:flex-row sm:text-left">
        <div className="flex-1">
          <h3 className="text-foreground text-sm font-semibold tracking-tight">
            Or run it yourself, free forever.
          </h3>
          <p className="text-muted-foreground mt-1 text-sm">
            The entire stack is Apache 2.0 with 100% feature parity. Your database,
            your domain, no limits.
          </p>
        </div>
...
const groups: Group[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { label: "Analytics retention", values: ["90 days", "2 years", "Forever"] },
...
function CellValue({ value }: { value: Cell }) {
  if (value === true)
    return (
      <span className="bg-live/15 inline-flex size-5 items-center justify-center rounded-full">
        <Check className="text-live size-3" />
      </span>
    )
  if (value === false) return <Minus className="text-muted-foreground/40 mx-auto size-3.5" />
  return <span className="text-foreground/90 text-sm tabular-nums">{value}</span>
}
  ...
  return (
    <div className="overflow-x-auto">
      <div className="min-w-[40rem]">
        {/* header */}
        <div className="border-border/60 grid grid-cols-[1.5fr_1fr_1fr_1fr] border-b">
          <div className="text-muted-foreground p-4 text-sm font-medium">Features</div>
          {cols.map((c) => (
            <div
              key={c}
              className={cn(
                "text-foreground p-4 text-center text-sm font-semibold",
                c === "Pro" && "bg-card border-border/60 rounded-t-xl border-x border-t",
              )}
            >
              {c}
            </div>
        ...
        {groups.map((g) => (
          <React.Fragment key={g.title}>
            <div className="border-border/40 grid grid-cols-[1.5fr_1fr_1fr_1fr] border-b">
              <div className="px-4 pt-8 pb-3">
                <h4 className="text-foreground text-sm font-semibold tracking-tight">
                  {g.title}
                </h4>
                <p className="text-muted-foreground mt-0.5 text-xs">{g.description}</p>
              </div>
              <div />
              <div className="bg-card border-border/60 border-x" />
              <div />
            </div>
            {g.rows.map((row) => (
              <div
                key={row.label}
                className="border-border/40 grid grid-cols-[1.5fr_1fr_1fr_1fr] border-b"
              >
                <div className="text-muted-foreground p-4 text-sm">{row.label}</div>
                {row.values.map((v, i) => (
                  <div
                    key={i}
                    className={cn(
                      "flex items-center justify-center p-4",
                      i === 1 && "bg-card border-border/60 border-x",
                    )}
                  >
                    <CellValue value={v} />
                  </div>
...
export function PricingFaq() {
  return (
    <div className="grid gap-10 lg:grid-cols-[1fr_1.6fr]">
      <div>
        <h2 className="text-foreground text-3xl font-semibold tracking-tight">FAQs</h2>
        <p className="text-muted-foreground mt-2 text-sm">Your questions, answered.</p>
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="py-4">
      ...
      >
        <p className="text-muted-foreground overflow-hidden text-sm leading-relaxed">{a}</p>
      </div>
    </div>
  )
}
